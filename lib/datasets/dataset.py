...
class LmdbDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = np.full((self.max_len,), self.char2id[self.PADDING], dtype=int)
        label_list = []
        for char in word:
...
def test():
    ...
    lmdb_path = "C://aster.pytorch-master//data//Case-Sensitive-Scene-Text-Recognition-Datasets-master//svt_train"
    # get num_samples
    label_file = os.path.join(lmdb_path, "all.txt")
    ...
    train_dataloader = data.DataLoader(
        ...
        shuffle=False,
      # num_workers=4, #under linux,it's ok to use multiprocess,but under windows,change to use multithreading
        num_workers=0,
