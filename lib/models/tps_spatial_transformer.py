...
def grid_sample(input, grid, canvas = None): # 采样器，根据网格对原图片进行转换，
...
class TPSSpatialTransformer(nn.Module):
  ...
  def __init__(self, output_image_size=None, num_control_points=None, margins=None):
    ...
    forward_kernel[:N, :N] = torch.clone(target_control_partial_repr)
    forward_kernel[:N, -3].fill_(1)
    forward_kernel[-3, :N].fill_(1)
    forward_kernel[:N, -2:] = torch.clone(target_control_points)
    forward_kernel[-2:, :N] = torch.clone(target_control_points.transpose(0, 1))
    ...
    Y = torch.true_divide(Y, self.target_height - 1) #带小数除法
  ...
  def forward(self, input, source_control_points):
    ...
    source_coordinate = torch.matmul(self.target_coordinate_repr, mapping_matrix.to(torch.float64))
    ...
    grid = 2.0 * grid - 1.0
    output_maps = grid_sample(input, grid, canvas=None) #应用双线性插值，把输入的tensor转换为指定大小
