...
def get_sinusoid_encoding(n_position, feat_dim, wave_length=10000):
  ...
  dim_range = torch.pow(wave_length, 2 * torch.true_divide((dim_range // 2) , feat_dim))
  # [n_position, feat_dim]
  angles = torch.true_divide(positions.unsqueeze(1) , dim_range.unsqueeze(0))
  angles = angles.float()
...
if __name__ == "__main__":
  ...
  net = ResNet_ASTER()
  encoder_feat = net(x)
  print(encoder_feat.size()) #torch.Size([3, 25, 512])