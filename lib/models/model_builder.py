...
from lib.models.stn_head import STNHead
...
class ModelBuilder(nn.Module):
  ...
  def __init__(self, arch, rec_num_classes, sDim, attDim, max_len_labels, eos, STN_ON=False):
    ...
    encoder_out_planes = self.encoder.out_planes #512
  ...
  def forward(self, input_dict):
    ...
    if self.training:
      ...
      #sPrev：rec_targets yPrev：rec_lengths
...
if __name__ == "__main__":
  ...
  #print(control_points.size())  # torch.Size([10, 20, 2])
  ...
  encoder = create("ResNet_ASTER",
                   with_lstm=True,
                   n_group=1)
  encoder_feats = encoder(x)
  encoder_feats = encoder_feats.contiguous()
  print(encoder_feats.size())#torch.Size([10, 25, 512])