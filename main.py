...
def get_data(data_dir, voc_type, max_len, num_samples, height, width, batch_size, workers, is_train, keep_ratio):
    ...
    if is_train:
        data_loader = DataLoader(dataset, batch_size=batch_size, num_workers=0,  # workers,
...
def get_dataloader(synthetic_dataset, real_dataset, height, width, batch_size, workers,
    ...
    data_loader = DataLoader(concated_dataset, batch_size=batch_size, num_workers=0,  # workers,
                             shuffle=False, pin_memory=False, drop_last=True, sampler=sampler,
                             collate_fn=AlignCollate(imgH=height, imgW=width, keep_ratio=keep_ratio))
...
def main(args):
    ...
    if args.evaluate:
        ...
        if len(args.vis_dir) > 0:
            ...
            vis_dir = None
        print('vis_dir:{0}'.format(vis_dir))
        start = time.time()
